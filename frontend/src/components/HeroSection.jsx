 ...
 * - normalizes many API shapes (src/url/path, title/caption, etc)
 * - ensures slide + img heights are forced so images are visible
 * - uses a gradient overlay that doesn't block clicks and keeps text readable
 */
...
const normalize = (item = {}, idx) => {
  ...
  const src = srcRaw
    ? isAbsolute
      ? srcRaw
      : `http://localhost:5000/${srcRaw.replace(/^\/?/, "")}`
    : "";
...
export default function HeroSection() {
    ...
    const fetchCarouselImages = async () => {
      try {
        ...
        const arr = Array.isArray(raw)
          ? raw
          : raw?.images ?? raw?.data ?? raw?.items ?? [];

        const normalized = (Array.isArray(arr) && arr.length ? arr : []).map(
          (it, i) => normalize(it, i)
        );
  ...
  return (
    <div className="w-full bg-white relative z-10">
      ...
      {/* carousel wrapper: enforce a height so slides fill it */}
...
>
   {image.buttonText || "Shop Now"} &gt;
                ...
                {/* bottom gradient to help text readability on lower area */}
      ...
      <div className="border-t border-b border-gray-200 py-4 bg-green-50">
        <div className="text-center">
          <p className="text-sm text-gray-700">
            <span className="font-semibold text-green-800">Sarah Johnson</span> purchased{" "}
            <span className="font-semibold">Monstera Deliciosa</span> 31 minutes ago
          </p>
        </div>
      </div>
    </div>
